...
namespace SolarLab.AdvertBoard.Mobile.Presentation.Infrastructure.Http
{
    ...
    public class AdvertApiClient(IHttpClientFactory factory) : IAdvertApiClient
    {
        ...
        public async Task<AdvertIdResponse> CreateDraftAsync(CreateAdvertDraftRequest request, string jwt)
        {
            _httpClient.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", jwt);

            var response = await _httpClient.PostAsJsonAsync("/api/adverts/drafts", request);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                if (problem != null && problem.Errors != null && problem.Errors.Count > 0)
                {
                    ...
                    var errorsText = string.Join("\n", problem.Errors.SelectMany(kv => kv.Value.Select(v => $"{kv.Key}: {v}")));
                    throw new HttpRequestException(errorsText);
                }

                var message = problem?.Detail ?? problem?.Title ?? $"Ошибка {response.StatusCode}";
                throw new HttpRequestException(message);
            }

            var result = await response.Content.ReadFromJsonAsync<AdvertIdResponse>();
            return result;
        }
